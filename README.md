...
```bash
git clone https://github.com/gitSanje/go-taskManiac.git
cd go-taskManiac
```
...
The backend uses environment variables prefixed with `TASK_`. Key variables include:

- `TASK_DATABASE_*` - PostgreSQL connection settings
- `TASK_SERVER_*` - Server configuration
- `TASK_AUTH_*` - Authentication settings
- `TASK_REDIS_*` - Redis connection
- `TASK_EMAIL_*` - Email service configuration
- `TASK_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

This taskManiac follows clean architecture principles:
